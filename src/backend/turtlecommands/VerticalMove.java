...
public abstract class VerticalMove extends Command {
	...
	public String computeVerticalMove(Turtle turtle, ArrayList<Command> inputs, boolean forward) {
		...
		double newX = turtle.getMyX() + ((forward) ?  xDisplacement : -1*xDisplacement);
		double newY = turtle.getMyY() + ((forward) ? yDisplacement : -1*yDisplacement);
		
		turtle.setMyPos(newX, newY);
